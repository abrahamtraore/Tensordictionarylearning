...
#from MiscellaneousFunctions import OnlineTensor
#from MiscellaneousFunctions import MethodsTSPen
...
from MiscellaneousFunctions.MethodsTSPen import GenerateTensorsNonnegative
from MiscellaneousFunctions.MethodsTSPen import  pool_init
...
def ExperimentToyGeneral(etavalues,Nonnegative,Numberofexamples,Minibatchsize,max_iter,step,alpha,theta,nbepochs,randomarray,trainratio,period,pool):    
    ...
    for l in range(L):
      ...
      for k in range(len(randomarray)):
         ...
         Ginittrain=np.random.normal(loc=0,scale=1/100,size=([len(Xtrain_set),eta,eta,eta]))
         ...
         for n in range(len(Xtrain_set)):
            Pre_existingG_settrain.append(np.maximum(Ginittrain[n,:,:,:],0))
            
      
         Pre_existingG_settest=[]
         for n in range(len(Xtest_set)):
            Pre_existingG_settest.append(np.maximum(Ginittest[n,:,:,:],0))
         
         Ltest=len(Xtest_set)
         
         Pre_existingfactors=[np.maximum(np.random.normal(loc=0,scale=1/10,size=(30,eta)),0),np.maximum(np.random.normal(loc=0,scale=1/10,size=(40,eta)),0),np.maximum(np.random.normal(loc=0,scale=1/10,size=(50,eta)),0)]
         Pre_existingP=[np.maximum(np.random.normal(loc=0,scale=1/10,size=(30,eta)),0),np.maximum(np.random.normal(loc=0,scale=1/10,size=(40,eta)),0),np.maximum(np.random.normal(loc=0,scale=1/10,size=(50,eta)),0)]
         Pre_existingQ=[np.maximum(np.random.normal(loc=0,scale=1/10,size=(eta,eta)),0),np.maximum(np.random.normal(loc=0,scale=1/10,size=(eta,eta)),0),np.maximum(np.random.normal(loc=0,scale=1/10,size=(eta,eta)),0)]
         ...
         Setting="MiniBatch" 
         print("Point IV")
         listoffactorsresult3=CyclicBlocCoordinateTucker_setWithPredefinedEpochs(Xtrain_set,Coretensorsize,Pre_existingfactors,Pre_existingG_settrain,Pre_existingP,Pre_existingQ,Nonnegative,Reprojectornot,Setting,Minibatchsize,step,alpha,theta,max_iter,epsilon,period,nbepochs,pool)        
         
         Gtest3=Sparse_coding(Xtest_set,Pre_existingG_settest,listoffactorsresult3,Nonnegative,Setting,step,max_iter,alpha,theta,epsilon,pool)
         error3=Error(Xtest_set,Gtest3,listoffactorsresult3,"MiniBatch",pool)
         fittingerror3=error3/Ltest
         rmse3=np.sqrt(error3/Ltest)
         mreminibatch=Mean_relative_error(Xtest_set,Gtest3,Operations_listmatrices(listoffactorsresult3,"Tensorize"),"MiniBatch",pool)
...
max_iter=20
Numberofexamples=12       #6000   
Minibatchsize=[2,2]          #[400,800,800]                                                      
alpha=np.power(10,2,dtype=float)   
theta=np.power(10,-2,dtype=float)   
period=int(max_iter/3)
randomarray=[1,2,3]
trainratio=1/3
pool=Pool(3,initializer=pool_init(),maxtasksperchild=1000000)        #Pool(3,initializer=pool_init(),maxtasksperchild=100000000)
RMSEminibatch,StdminibatchRMSE,Fittingminibatch,Stdminibatchfitting,MREminibatch,Stdminibatchmre=ExperimentToyGeneral(etavalues,Nonnegative,Numberofexamples,Minibatchsize,max_iter,step,alpha,theta,nbepochs,randomarray,trainratio,period,pool)
