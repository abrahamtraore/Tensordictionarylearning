...
import numpy as np
np.random.seed(1)
#np.random.seed(2): this was the initial random
import random
...
def CyclicBlocCoordinateTucker_single(X,Coretensorsize,Pre_existingfactors,Pre_existingG,Pre_existingP,Pre_existingQ,Nonnegative,Setting,t,step,alpha,theta,max_iter,epsilon,period,pool):#All the parameters are arrays,
    ...
    if(Setting=="Single"):
       ...
       Inferredfactorsold=[]
       print("We are in CyclicBlocCoordinateTucker_single")
       nb_iter=0    
       ...
       while(nb_iter<max_iter):
          ...
          error=Error(tl.tensor(X),tl.tensor(G),Operations_listmatrices(Inferredfactorsnew,"Tensorize"),Setting,pool)#+alpha*theta*NORM(Pre_existingG,Setting,1)+(alpha*(1-theta)/2)*np.sum(np.array(Operations_listmatrices(Inferredfactorsnew,"NormII")))
    ...
    if(Setting=="MiniBatch"):
       ...
       while(nb_iter<max_iter):
          ...
          error=Error(Operations_listmatrices(X,"Tensorize"),Operations_listmatrices(G,"Tensorize"),Operations_listmatrices(Inferredfactorsnew,"Tensorize"),Setting,pool)#+alpha*theta*NORM(Pre_existingG,Setting,1)+(alpha*(1-theta)/2)*np.sum(np.array(Operations_listmatrices(Inferredfactorsnew,"NormII")))
