...
def Factorupdateproblem(X,G,Ainit,listoffactorsmatrices,Nonnegative,alpha,theta,n,maxiter,step,epsilon):
    ...
    while(nbiter<maxiter):
        ...
        Anew=Aold-step*derivativeDict(X,G,Aold,listoffactorsmatrices,alpha,theta,n)
        if(Nonnegative==True):
...
def TuckerBatch(X,Coretensorsize,max_iter,listoffactorsinit,Ginit,Nonnegative,Reprojectornot,alpha,theta,step,epsilon):
    ...
    while (nbiter<max_iter): 
        ...
        for n in range(N-1):
            ...
            Anew=Factorupdateproblem(tl.tensor(X),Gnew,Aold,listoffactorsnew,Nonnegative,alpha,theta,n+1,max_iter,step,epsilon)
...
def TuckerBatchfull(X,Coretensorsize,max_iter,listoffactorsinit,Ginit,Nonnegative,Reprojectornot,alpha,theta,step,epsilon):
    N=len(list(X.shape))
    listoffactorsnew=list(listoffactorsinit)   
    listoffactorsnew=Operations_listmatrices(listoffactorsnew,"Tensorize")
    listoffactorsold=[]
    ...
    while (nbiter<max_iter): 
        ...
        for n in range(N):
            ...
            Anew=Factorupdateproblem(tl.tensor(X),Gnew,Aold,listoffactorsnew,Nonnegative,alpha,theta,n,max_iter,step,epsilon)
